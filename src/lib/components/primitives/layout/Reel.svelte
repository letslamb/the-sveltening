<script>
  ...
  import { detectTouch } from '$lib/js/utils.js'
...
<style>
  ...
  /* Exposed as CSS variables
    --reel-height
    --reel-focus-color
    --reel-scrollbar-height
    --reel-scrollbar-color 
    --reel-scrollbar-track-color
    --reel-spacing => this controls the space between reel children *and* the space between the children &
    scrollbar
  */
  ...
  .reel {
    ...
    overflow-x: auto;
    overflow-y: hidden;
    scrollbar-color: var(--reel-scrollbar-color, #fff) var(--reel-scrollbar-track-color, #000);
  }

  .reel::-webkit-scrollbar {
    height: var(--reel-scrollbar-height, 1rem);
  }

  .reel::-webkit-scrollbar-track {
    background-color: var(--reel-scrollbar-track-color, #000);
  }

  .reel::-webkit-scrollbar-thumb {
    background-color: var(--reel-scrollbar-track-color, #000);
    background-image: linear-gradient(
      var(--reel-scrollbar-track-color, #000) 0, 
      var(--reel-scrollbar-track-color, #000) 25%, 
      var(--reel-scrollbar-color, #fff) 25%, 
      var(--reel-scrollbar-color, #fff) 75%, 
      var(--reel-scrollbar-track-color, #000) 75%
    );
  }

  /* set flex-grow, flex-shrink & flex-basis for children of the reel, but exclude the visually hidden <span> */
  :global(.reel > *:not(span)) {
    flex: 0 0 25rem;
  }
  ...
  [aria-labelledby="reel-label"]:focus {
    outline: 4px solid var(--reel-focus-color, skyBlue);
  }
